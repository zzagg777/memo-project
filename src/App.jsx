...
// context : 1.만들기 createContext() > 2.제공하기 <Provider value={}> >> 3.사용하기 useContext()
...
function App() {
  ...
  const handleUpdate = async (id, changes) => {
    ...
    try {
      ...
      // console.log(updated);
      // setMemos((prev) => prev.map((memo) => (memo.id === id ? updated : memo)));
      dispatch({ type: ACTION_TYPE.update, payload: updated });
    } catch (err) {
      dispatch({
        type: ACTION_TYPE.error,
        payload: "메모 수정 실패",
  ...
  const handlePinned = async (id, changes) => {
    ...
    try {
      ...
      // });
      // setMemos(newMemo);
      dispatch({ type: ACTION_TYPE.pinned, payload: id });
    } catch (err) {
      // setError("고정에 실패했습니다.");
      ...
      >
        ...
        >
          ...
          <Routes>
            ...
            <Route path="/memos/new" element={<WritePage />} />
            <Route path={"/memos/:id"} element={<ViewPage />} />
            <Route path={"/memos/:id/edit"} element={<WritePage />} />
